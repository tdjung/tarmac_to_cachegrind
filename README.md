...
| 이벤트 | 의미 |
|---|---|
| `Tests` | 해당 PC를 한 번 이상 실행한 정상 처리 테스트 수 |
...
전체 450개 중 테스트 17, 305만 실행하지 않은 PC는 `Tests=448`,
...
않았지만 EXC/IS 이벤트가 있는 정상 로그는 실행된 PC의 미커버 테스트로 포함합니다.
...
커버·미커버 이벤트는 **각 instruction PC별로** 기록합니다. 같은 소스 라인에 여러 PC가
있어도 각 PC의 실행 테스트 집합을 따로 계산합니다. 대표 PC에만 표시하거나 라인의
합집합을 복제하지 않습니다. `Ir`과 소스 라인 매핑은 기존 값을 유지합니다.

**해당 PC의 total `Ir=0`이면 `Tests`, `Covered1~5`, `CoveredSet`,
`Uncovered1~5`, `UncoveredSet` 모두 0입니다.** 이 PC의 미커버 목록은 생성하지 않습니다.
`Ir>0`인 PC에 대해서만 정상 처리 테스트 중 실행하지 않은 테스트를 `Uncovered`에 기록합니다.

예를 들어 같은 소스 라인의 `0x100`은 미실행이고 `0x102`를 테스트 1만 실행했다면,
`0x100`의 추가 이벤트는 전부 0이고 `0x102`의 `Tests=1`, `Covered1=1`입니다.
테스트 2도 정상 처리되었지만 `0x102`를 실행하지 않았다면 그 PC의 `Uncovered1=2`입니다.

인덱스 및 집합 번호는 **instruction 행에서 읽는 식별자**입니다. 뷰어가 여러 PC의
이벤트를 소스 라인이나 함수로 합산하면 번호가 더해지므로 그 합계는 테스트 번호로
해석할 수 없습니다. `Tests` 합계 역시 라인/함수의 고유 테스트 수가 아닙니다.
summary는 형식에 맞춰 열별 합계를 기록하지만 인덱스 합계에는 의미가 없습니다.
커버리지 색상은 `Ir`을 기준으로 확인하세요. `Covered5=0`은 다섯 번째 테스트가 없다는
뜻이므로 미실행 판정에 사용하면 안 됩니다. 실제 사용자 뷰어의 표시는 별도 확인이 필요합니다.

목록 JSON은 `version=2`, `coverage_unit="instruction_pc"`로 새 집계 단위를 명시합니다.
집합 ID와 테스트 ID 조회 방식은 동일합니다. 기존 total과 새 목록을 섞지 말고 함께 재생성하세요.

#### 추가 처리 비용

부모가 파일별 sparse PC 집계에 테스트 비트를 OR하고, 최종 total 생성 때 PC별로
기록합니다. 성공 테스트 집합의 차집합으로 미커버를 계산합니다. 명령어마다 공유 락을
...
- 같은 라인의 PC별 독립 집계와 Ir=0의 모든 추가 이벤트 0 처리, 450개 테스트의 커버/미커버 목록, 넘침 집합 재사용,
